...
\begin{enumerate}[leftmargin=*,topsep=0pt,itemsep=2pt]
    ...
    \item \textbf{Practical benefit}: Specialized populations outperform generalists by +55.5pp $\pm$ 8.7pp ($n=3$) with 5-7 task break-even (Section~\ref{sec:realworld}).
\end{enumerate}
...
\begin{table}[h]
\centering
\caption{Practical benefit comparison ($n=3$ runs). Oracle routing achieves +55.5pp $\pm$ 8.7pp improvement.}
\label{tab:practical}
\begin{tabular}{lccc}
\toprule
\textbf{Condition} & \textbf{Accuracy} & \textbf{API Calls} & \textbf{$\Delta$} \\
\midrule
SINGLE\_GENERALIST & 27.8\% & 24 & -- \\
\textbf{ORACLE\_ROUTING} & \textbf{83.3\%} & 24 & \textbf{+55.5pp} \\
CONFIDENCE\_ROUTING & 37.5\% & 216 & +9.7pp \\
ENSEMBLE & 45.8\% & 192 & +18.0pp \\
\bottomrule
\end{tabular}
\end{table}

Oracle routing (perfect task-type labels) achieves \textbf{+55.5 $\pm$ 8.7 percentage points improvement} ($n=3$) at no additional cost---demonstrating the value of specialization when routing is accurate.
...
\begin{itemize}[leftmargin=*,topsep=0pt,itemsep=2pt]
    ...
    \item \textbf{Accuracy improvement}: +55.5pp $\pm$ 8.7pp (Oracle routing, $n=3$)
\end{itemize}
...
\begin{enumerate}[leftmargin=*,topsep=0pt,itemsep=2pt]
    \item \textbf{Routing overhead}: Practical benefit requires task-type prediction. Oracle routing (+55.5pp) is an upper bound.
