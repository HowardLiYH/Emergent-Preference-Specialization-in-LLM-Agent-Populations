...
\begin{table}[h]
\centering
\caption{Practical benefit comparison. Oracle routing achieves +58.3pp improvement at no additional API cost.}
\label{tab:practical}
\begin{tabular}{lccc}
\toprule
...
Oracle routing (perfect task-type labels) achieves \textbf{+58.3 percentage points improvement} at no additional cost---demonstrating the value of specialization when routing is accurate.
...
\begin{enumerate}[leftmargin=*,topsep=0pt,itemsep=2pt]
    \item \textbf{Routing overhead}: Practical benefit requires task-type prediction. Oracle routing (+58.3pp) is an upper bound.
