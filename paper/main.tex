...
\begin{enumerate}[leftmargin=*,topsep=0pt,itemsep=2pt]
    ...
    \item \textbf{Practical benefit}: Evolved specialists achieve the theoretical ceiling (100\% on matched tasks); oracle routing unlocks +64.2pp $\pm$ 2.3pp improvement ($n=5$) with 5-7 task break-even (Section~\ref{sec:realworld}).
\end{enumerate}
...
We demonstrate that LLM agent populations can develop specialized preferences through competitive selection alone. Our 70.7\% causality validation rate (Cohen's $d=2.66$) confirms that evolved prompts \textit{cause} performance differences. Crucially, specialists achieve the theoretical performance ceiling (100\%) on matched tasks, confirming complete---not partial---specialization. Oracle routing unlocks this value, yielding +64.2pp $\pm$ 2.3pp improvement ($n=5$) with 5-7 task break-even. This work establishes prompt-based specialization as a viable paradigm for creating diverse, complementary LLM agent populations. Future work should explore dynamic niche discovery and self-routing mechanisms to approximate oracle routing in deployment.
