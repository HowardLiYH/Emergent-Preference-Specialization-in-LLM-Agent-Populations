...
\begin{enumerate}[leftmargin=*,topsep=0pt,itemsep=2pt]
    ...
    \item \textbf{Practical benefit}: Specialized populations outperform generalists by +64.2pp $\pm$ 2.3pp ($n=5$, 100\% oracle accuracy) with 5-7 task break-even (Section~\ref{sec:realworld}).
\end{enumerate}
...
\begin{table}[h]
\centering
\caption{Practical benefit comparison ($n=5$ runs, consistent configuration). Oracle routing achieves +64.2pp $\pm$ 2.3pp improvement with 100\% accuracy in all trials.}
\label{tab:practical}
\begin{tabular}{lccc}
\toprule
...
Oracle routing (perfect task-type labels) achieves \textbf{+64.2 $\pm$ 2.3 percentage points improvement} ($n=5$, 95\% CI: [61.3, 67.0], 100\% oracle accuracy in all trials) at no additional cost---demonstrating the value of specialization when routing is accurate.
...
\begin{itemize}[leftmargin=*,topsep=0pt,itemsep=2pt]
    ...
    \item \textbf{Accuracy improvement}: +64.2pp $\pm$ 2.3pp (Oracle routing, $n=5$, 100\% ceiling)
\end{itemize}
...
\begin{enumerate}[leftmargin=*,topsep=0pt,itemsep=2pt]
    \item \textbf{Routing overhead}: Practical benefit requires task-type prediction. Oracle routing (+64.2pp) is an upper bound.
