...
\begin{enumerate}[leftmargin=*,topsep=0pt,itemsep=2pt]
    ...
    \item \textbf{Practical benefit}: Specialized populations outperform generalists by +52.0pp on average with 5-7 task break-even (Section~\ref{sec:realworld}).
\end{enumerate}
...
\begin{table}[h]
\centering
\caption{Practical benefit comparison (average of $n=2$ runs after content filtering fix). Oracle routing achieves +52.0pp improvement on average.}
\label{tab:practical}
\begin{tabular}{lccc}
\toprule
\textbf{Condition} & \textbf{Accuracy} & \textbf{API Calls} & \textbf{$\Delta$} \\
\midrule
SINGLE\_GENERALIST & 22.9\% & 24 & -- \\
\textbf{ORACLE\_ROUTING} & \textbf{75.0\%} & 24 & \textbf{+52.0pp} \\
CONFIDENCE\_ROUTING & 20.8\% & 216 & +0.0pp \\
ENSEMBLE & 29.2\% & 192 & +6.3pp \\
\bottomrule
\end{tabular}
\end{table}

Oracle routing (perfect task-type labels) achieves \textbf{+52.0 percentage points improvement} on average (range: +45.8pp to +58.3pp) at no additional cost---demonstrating the value of specialization when routing is accurate.
...
\begin{itemize}[leftmargin=*,topsep=0pt,itemsep=2pt]
    ...
    \item \textbf{Accuracy improvement}: +52.0pp average (Oracle routing, range: +45.8pp to +58.3pp)
\end{itemize}
...
\begin{enumerate}[leftmargin=*,topsep=0pt,itemsep=2pt]
    \item \textbf{Routing overhead}: Practical benefit requires task-type prediction. Oracle routing (+52.0pp average) is an upper bound.
