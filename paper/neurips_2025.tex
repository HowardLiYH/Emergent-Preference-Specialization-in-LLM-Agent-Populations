...
\midrule
\textbf{Average} & \textbf{0.91} & \textbf{0.31} & \textbf{65.7\%} \\
\bottomrule
\end{tabular}
\end{table}

Both models exceed the 30\% gap threshold, confirming that prompt specialization is not model-specific but a general property of LLM agents under competitive selection.
...
\begin{enumerate}
    ...
    \item Synthetic rules are simpler than real-world tasks; transfer to complex domains untested.
    \item Cross-LLM validation limited to two models (Gemini, GPT-4o-mini); broader coverage needed.
    \item Competition mechanism requires sufficient population size for diversity.
\end{enumerate}
