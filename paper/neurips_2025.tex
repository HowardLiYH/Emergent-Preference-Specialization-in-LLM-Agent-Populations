...
\begin{abstract}
We demonstrate that populations of initially identical LLM agents can develop specialized \emph{preferences} through competitive selection, without any gradient-based training or external reward shaping. Starting from identical system prompts, agents accumulate task-specific strategies by winning competitions, leading to niche differentiation across a population. Our key contributions are: (1) \textbf{Preference Emergence}: Agents naturally develop distinct preferences for different task types (8/8 synthetic rules covered by specialists); (2) \textbf{Causal Mechanism}: Prompt swap experiments demonstrate that accumulated prompts \emph{cause} performance differences (75.5\% pass rate, 95\% CI [64.6\%, 86.4\%]); (3) \textbf{Prompt Design Insight}: Counter-intuitively, concise prompts ($\sim$30 chars) outperform verbose prompts ($\sim$900 chars) by 24\%; (4) \textbf{Robust Baselines}: Correct specialists achieve 87\% accuracy vs. 27\% for no-prompt baseline (+60\%). Results are validated across 5 seeds and scale to 48 agents. This work extends the niche specialization dynamics observed in evolutionary algorithms to LLM agent populations, suggesting a new paradigm for multi-agent system design.
\end{abstract}
...
\begin{table}[h]
\caption{Multi-Seed Swap Test Results (n=5 seeds)}
\label{tab:multiseed}
\centering
\begin{tabular}{lcccc}
\toprule
...
\midrule
NO\_PROMPT & 27.0\% & -60.0\% \\
RANDOM\_PROMPT & 32.0\% & -55.0\% \\
WRONG\_PROMPT & 38.0\% & -49.0\% \\
CORRECT\_PROMPT & \textbf{87.0\%} & - \\
\bottomrule
\end{tabular}
\end{table}
