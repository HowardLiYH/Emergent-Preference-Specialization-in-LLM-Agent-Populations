...
\begin{enumerate}
    \item \textbf{Causality Is Demonstrated}: 70.7\% pass rate across 10 seeds (95\% CI: [68.3\%, 73.1\%]).
...
We run swap tests across 10 random seeds with a unified model (gemini-2.5-flash) to ensure reproducibility (Table~\ref{tab:multiseed}).

\begin{table}[h]
\caption{Multi-Seed Swap Test Results (n=10 seeds, UNIFIED)}
\label{tab:multiseed}
\centering
\begin{tabular}{lcccc}
\toprule
...
\midrule
Pass Rate & 70.7\% & 0.034 & [68.3\%, 73.1\%] \\
\bottomrule
\end{tabular}
\footnotesize{All 10 seeds: gemini-2.5-flash (unified model). CI width reduced by 84\% vs previous.}
\end{table}
...
We demonstrate that LLM agent populations can develop specialized preferences through competitive selection alone. The key insights are: (1) prompts causally drive specialization (70.7\% pass rate, validated across 10 seeds with unified model, 95\% CI: [68.3\%, 73.1\%]); (2) concise prompts outperform verbose ones by 24\%; (3) correct specialists achieve 60\% higher accuracy than no-prompt baseline; (4) performance scales robustly from 8 to 48 agents. This work suggests a new paradigm for multi-agent LLM systems: let competition produce specialists naturally---and keep the prompts simple.
