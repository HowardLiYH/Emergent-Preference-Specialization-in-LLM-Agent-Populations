...
\normalsize From First Principles to Best Paper Candidate}
...
\node[draw, rectangle, minimum width=3cm, minimum height=1cm] (before) {Single Generalist\\20.8\% accuracy};
\node[draw, rectangle, minimum width=3cm, minimum height=1cm, right=3cm of before] (after) {Specialized Population\\79.2\% accuracy};
\draw[->, thick] (before) -- node[above] {+58.3pp} (after);
\end{tikzpicture}
\end{center}
...
\begin{warning}
\textbf{+58.3pp at No Extra API Cost!}
...
\begin{enumerate}
    ...
    \item \textbf{Massive practical benefit}: +58.3pp at no extra cost
...
\begin{itemize}
    \item +58.3pp accuracy at no extra cost
