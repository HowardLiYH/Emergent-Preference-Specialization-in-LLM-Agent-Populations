...
Accuracy Gain & +52.0pp (avg) & Oracle routing vs generalist \\
...
\node[draw, rectangle, minimum width=3cm, minimum height=1cm] (before) {Single Generalist\\22.9\% accuracy};
\node[draw, rectangle, minimum width=3cm, minimum height=1cm, right=3cm of before] (after) {Specialized Population\\75.0\% accuracy};
\draw[->, thick] (before) -- node[above] {+52.0pp} (after);
\end{tikzpicture}
\end{center}
...
\begin{warning}
\textbf{+52.0pp at No Extra API Cost (avg of 2 runs)!}
...
Accuracy Improvement & +52.0pp (avg) \\
ROI & Excellent \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Massive practical benefit}: +52.0pp average at no extra cost
...
\begin{itemize}
    \item +52.0pp accuracy on average at no extra cost
